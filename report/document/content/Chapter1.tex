...
This report studies a hybrid prediction pipeline that combines descriptor
features with knowledge graph representation learning. The first stage learns
entity representations from biomedical triples using knowledge graph embedding
(KGE) models. The second stage combines these learned representations with drug
and protein descriptors and trains a neural factorization machine (NFM) for
final DTI classification.
...
The central motivation is that DTI prediction is not only a molecular matching
problem and not only a graph completion problem. A drug--target pair is better
represented when direct molecular or sequence evidence is considered together
with biomedical context from the surrounding knowledge graph.
...
\cite{YangYHGD14a}. However, DistMult has limited expressiveness because
...
multi-relational graphs \cite{VashishthSNT20}. This is especially
...
settings \cite{0001C17}. This makes NFM a suitable candidate for combining
...
This gap motivates the hybrid pipeline evaluated in this report. A
descriptor-only model may miss relational evidence, while a graph-only model may
ignore useful entity-level features. KGE models are therefore used to learn
biomedical graph representations, and an NFM model combines those
representations with descriptor features for supervised DTI prediction.
...
\begin{itemize}
    ...
    \item to evaluate the model using ROC-AUC and PR-AUC on the Yamanishi08
    warm-start settings;
...
The study focuses on computational prediction rather than experimental
validation. Model outputs are probability scores for candidate drug--target
pairs, and should be interpreted as prioritization signals rather than
confirmed biological interactions.

The experimental report focuses on the \textit{Yamanishi08} benchmark and its
1:1 and 1:10 warm-start settings. The codebase contains loaders for additional
biomedical datasets, but they are not used as evidence for the conclusions in
this report. The main pipeline is implemented in Python using PyTorch for
neural model training and PyKEEN for knowledge graph embedding models.
...
\begin{itemize}
    \item a reproducible implementation of a KGE-NFM pipeline for DTI prediction;

    \item an experimental comparison between lightweight and more expressive KGE
    models in the downstream DTI setting;

    \item an evaluation framework using fold-based prediction outputs,
    ROC-AUC, PR-AUC, hyperparameter tuning, and ablation analysis;
